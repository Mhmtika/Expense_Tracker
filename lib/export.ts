...
export async function exportToPDF(expenses: Expense[], filename = "expenses"): Promise<void> {
  ...
  doc.setFontSize(9);
  doc.text(`Records: ${expenses.length}`, 20, 44);
  doc.text(`Total: ${formatCurrency(total)}`, 90, 44);
  if (expenses.length > 0) {
    const dates = expenses.map((e) => e.date).sort();
    doc.text(`${formatDate(dates[0])} – ${formatDate(dates[dates.length - 1])}`, 148, 44);
  }
